...
parser.add_argument("-r","--relation_dim",type = int, default = 8)#如果为celoss就改成2，triplet loss改成1，其他loss都是8，
parser.add_argument("-b","--batch_size",type = int, default = 32)
parser.add_argument("-e","--episode",type = int, default= 500)   # 调参
parser.add_argument("-l","--learning_rate", type = float, default = 0.0005)  # 调参
parser.add_argument("--config",type = str,default='./configs/configs.yml')
parser.add_argument("-g","--gpu",type=int, default=0)

parser.add_argument("-s","--select_split",type = int, default=3,help='1,2,3,4,5')   # 调参
parser.add_argument("-k","--k_neg_pair",type = int, default=1)   # 调参


# Hyper Parameters

parser.add_argument("-t","--dataset",type = str, default='KINFACE2',help='TSKINFACE or KINFACE1 or KINFACE2') # 调参
...
parser.add_argument("--discriminator",type = bool, default= False ,help='')#新增可调参数
...
if args.dataset == 'KINFACE2':
    from data.loader_kinface2 import load_kinface,get_train_loader_kinface2,make_test_indices,kinface_test_dataset
...
if args.conv == 'conv1234':
    ...
    # from conv.related_net_duikang import CNNEncoder_1234 as CNNEncoder
    # from conv.related_net_duikang import RelationNetwork_1234 as RelationNetwork
    # from conv.related_net_duikang import Domain_RelationNetwork_1234_0 as Domain_RelationNetwork
...
def main():
    ...
    if args.dataset == 'KINFACE2':
        related_list = get_train_loader_kinface2(args.dataset,SELECT_SPLIT,COLLECTION,BATCH_SIZE,args.picroot)
    ...
    if args.pretrained:
        feature_encoder_parents.load_state_dict(torch.load(args.feature_encoder))
        feature_encoder_children.load_state_dict(torch.load(args.feature_encoder))
        relation_network.load_state_dict(torch.load(args.relation_network))
        domain_relation_network.load_state_dict(torch.load(args.domain_relation_network))
    ...
    MV_loss_optim = torch.optim.Adam(MV.parameters(), lr=0.0001)
    ...
    for episode in range(EPISODE):
        ...
        if args.conv == 'conv1234':
            parent_features1, parent_features2, parent_features3, parent_features4 = feature_encoder_parents(
                parent_features)
            child_features1, child_features2, child_features3, child_features4 = feature_encoder_children(
                child_features)

            p1, p2, p3, p4, c1, c2, c3, c4, new_domain_labels = conv_process_1234_test(parent_features1,
                                                                                       parent_features2,
                                                                                       parent_features3,
                                                                                       parent_features4,
                                                                                       child_features1, child_features2,
                                                                                       child_features3, child_features4,
                                                                                       child_list, batch_labels[1],
                                                                                       K_PAIR, BATCH_SIZE, FEATURE_DIM,
                                                                                       GPU)
        ...
        if args.loss == 'MV':
            ...
            from conv.loss_func import loss_final
            loss = loss_final(distance, labels, loss_type='HardMining', criteria=None) + loss_final(distance, labels,
                                                                                                    loss_type='FocalLoss',
                                                                                                    criteria=None)
        elif args.loss == 'CE':
            ...
            loss = torch.sum(
                torch.clamp(torch.max(relation_neg, dim=1)[0] - torch.max(relation_pos, dim=1)[0] + 0.5, 0)) / 32
            ...
            if args.dataset=='KINFACE2':
                ...
                if Mean > last_Mean:
                    ...
                    if args.is_save_weights:
                            ...
                            torch.save(feature_encoder_parents.state_dict(),
                                join(args.modelpath,"feature_encoder_parents"+args.dataset+"_"+args.addtion +"_K_PAIR_"+str(K_PAIR)+"_SPLIT_"+ str(SELECT_SPLIT) + ".pth"))
                            torch.save(feature_encoder_children.state_dict(),
                                       join(args.modelpath,
                                            "feature_encoder_children" + args.dataset + "_" + args.addtion + "_K_PAIR_" + str(
                                                K_PAIR) + "_SPLIT_" + str(SELECT_SPLIT) + ".pth"))
                            torch.save(relation_network.state_dict(),
                                join(args.modelpath, "relation_network_"+args.dataset+"_"+args.addtion +"_K_PAIR_"+str(K_PAIR)+"_SPLIT_"+ str(SELECT_SPLIT) +".pth"))
                            if args.loss == 'MV':
