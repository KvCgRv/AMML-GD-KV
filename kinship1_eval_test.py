...
parser.add_argument("-r","--relation_dim",type = int, default = 2)#loss维度,如果使用ce就把维度换成2，triplet就换成1
parser.add_argument("-b","--batch_size",type = int, default = 32)
parser.add_argument("--config",type = str,default='./configs/configs.yml')
parser.add_argument("-g","--gpu",type=int, default=0)
parser.add_argument("-s","--select_split",type = int, default=4,help='1,2,3,4,5')   # 调参
...
parser.add_argument("--loss",type = str, default= 'CE',help='triplet or CE or MV')
...
def main():
    ...
    if args.pretrained:
        feature_encoder_parents.load_state_dict(
            torch.load('KINFACE2_model/feature_encoder_parentsKINFACE1_KINFACE1_conv1234_CE_K_PAIR_1_SPLIT_4.pth'))
        feature_encoder_children.load_state_dict(
            torch.load('KINFACE2_model/feature_encoder_childrenKINFACE1_KINFACE1_conv1234_CE_K_PAIR_1_SPLIT_4.pth'))
        relation_network.load_state_dict(
            torch.load('KINFACE2_model/relation_network_KINFACE1_KINFACE1_conv1234_CE_K_PAIR_1_SPLIT_4.pth'))
        if args.loss == 'MV':
            MV.load_state_dict(
                torch.load('KINFACE2_model/MVLossKINFACE1_KINFACE1_conv1234_MV_K_PAIR_1_SPLIT_2.pth')
            )
